...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left: Branding Panel */}
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-amber-600/20 via-background to-background items-center justify-center p-12">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg max-w-md">Continue your German learning journey right where you left off.</p>
          <div className="flex items-center justify-center gap-2 mt-8 text-amber-400 text-sm font-medium">
            <Sparkles className="w-4 h-4" />
            8,000+ words · 200+ lessons · AI-powered
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative">
        ...
        >
          ...
          <p className="text-muted-foreground mb-8">Log in to your StartGerman account</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label className="text-sm font-medium text-muted-foreground" htmlFor="email">Email</label>
              <Input
                id="email" type="email" placeholder="johannes@example.com"
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link href="#" className="text-sm text-amber-400 hover:text-amber-300 transition-colors">Forgot?</Link>
              </div>
              <Input
            ...
            <Button type="submit" className="w-full h-12 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-black font-bold hover:shadow-lg hover:shadow-amber-500/25 transition-all hover:scale-[1.02] border-0" disabled={isLoading}>
              {isLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Logging in...</> : 'Log In'}
            </Button>
          </form>

          <p className="text-center text-sm text-muted-foreground mt-8">
            Don't have an account?{' '}
            <Link href="/register" className="text-amber-400 hover:text-amber-300 font-medium transition-colors">Sign up</Link>
          </p>
        </motion.div>
      </div>
    </div>
