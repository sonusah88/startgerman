...
import { generateDailyStudyPlan } from '@/actions/study-plan';
...
export default function LessonPlayerPage() {
  ...
  useEffect(() => {
    async function loadSteps() {
      const steps = await generateDailyStudyPlan();
          ...
          >
            {step.type === 'input' && (
              <div className="text-center space-y-6">
                ...
                <div className="glass-card rounded-3xl p-10 inline-block mx-auto relative group">
                  <div className="flex items-center justify-center gap-4 mb-3">
                    <div className="text-5xl md:text-6xl font-bold text-foreground">{step.content.word}</div>
                    <button 
                      onClick={() => handleSpeak(step.content.word)}
                      disabled={isSpeaking}
                      className={`p-3 rounded-full transition-all ${isSpeaking ? 'bg-amber-500/20 text-amber-500' : 'bg-white/5 text-muted-foreground hover:text-foreground hover:bg-white/10'}`}
                    >
                      <Volume2 className="w-6 h-6" />
                    </button>
                  </div>
                  <div className="text-2xl text-muted-foreground">{step.content.meaning}</div>
                </div>
                <div className="glass-subtle rounded-2xl p-5 max-w-md mx-auto">
